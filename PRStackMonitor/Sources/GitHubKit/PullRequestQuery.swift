...
enum PullRequestQuery {
    /// The search, selecting ``fields`` on every node it finds.
    ///
    ...
    /// Everything a row is drawn from, as a fragment rather than as a selection set.
    ///
    /// A fragment because there are two queries that need it: this one's search, and the
    /// priority refresh that asks for named pull requests by id (``PriorityRefresh``). The
    /// two must select the *same* fields — a row arriving from the refresh with, say, no
    /// reviewers would redraw differently from the same row arriving from the search, and
    /// the unread digest would flip on every poll as the two spellings alternated.
    ///
    ...
    ///   which is wrong the moment the token type changes. The field is free. (It is
    ///   selected by the queries rather than by this fragment: it is a root field, not a
    ///   pull request field.)
